...
def medianOfFour(ar,left,gap):
    ...
    if my_cmp(ar[pos1],ar[pos2]) <= 0:
        if my_cmp(ar[pos2],ar[pos3]) <= 0:
            if my_cmp(ar[pos2],ar[pos4]) <= 0:
                if my_cmp(ar[pos3],ar[pos4]) > 0:
                    ar[pos3],ar[pos4] = swap(ar[pos3],ar[pos4])
            else:
                ar[pos2],ar[pos3] = swap(ar[pos2],ar[pos3])
        else:
            if my_cmp(ar[pos1],ar[pos3]) <= 0:
                if my_cmp(ar[pos3],ar[pos4]) <= 0:
                    if my_cmp(ar[pos2],ar[pos4]) <= 0:
                        ar[pos2],ar[pos3] = swap(ar[pos2],ar[pos3])
                    else:
